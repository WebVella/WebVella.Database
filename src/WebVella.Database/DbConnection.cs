...
public interface IDbConnection : IDisposable, IAsyncDisposable
{
    ...
    /// <summary>
    /// Asynchronously rolls back the current transaction, or reverts to the previous savepoint 
    ...
    /// <param name="commandType">The type of the command (e.g., <see cref="CommandType.Text"/>,
    /// <see cref="CommandType.StoredProcedure"/>).</param>
    /// <param name="parameters">Optional parameters for the command.</param>
	/// <returns>A configured <see cref="NpgsqlCommand"/> instance.</returns>
	public NpgsqlCommand CreateCommand(
		string sql, CommandType commandType = CommandType.Text, params NpgsqlParameter[] parameters);
...
/// <summary>
/// Provides an implementation of <see cref="IDbConnection"/> for managing PostgreSQL database connections,
...
internal class DbConnection : IDbConnection
{
	...
	private readonly DbConnectionContext _currentContext;

	/// <summary>
	/// Initializes a new instance of DbConnection with an existing transaction.
	/// </summary>
	...
	internal DbConnection(NpgsqlTransaction transaction, DbConnectionContext connectionContext)
	{
	...
	/// <summary>
	/// Initializes a new instance of DbConnection with a connection string.
	/// </summary>
	/// <param name="connectionString">The connection string for the database.</param>
	/// <param name="connectionContext">The associated connection context.</param>
    internal DbConnection(string connectionString, DbConnectionContext connectionContext)
    {
        _currentContext = connectionContext;
        _transaction = null;
	...
	/// </summary>
	/// <param name="connectionContext">The associated connection context.</param>
    private DbConnection(DbConnectionContext connectionContext)
    {
        _currentContext = connectionContext;
    }
	...
	/// <summary>
	/// Asynchronously creates a new instance of DbConnection with a connection string.
	/// </summary>
	/// <param name="connectionString">The connection string for the database.</param>
	/// <param name="connectionContext">The associated connection context.</param>
	/// <returns>A configured DbConnection instance.</returns>
	internal static async Task<DbConnection> CreateAsync(string connectionString, DbConnectionContext connectionContext)
	{
		var conn = new DbConnection(connectionContext);
	...
	/// <inheritdoc/>
	public NpgsqlCommand CreateCommand(
		string sql, CommandType commandType = CommandType.Text, NpgsqlParameter[]? parameters = null)
	{
